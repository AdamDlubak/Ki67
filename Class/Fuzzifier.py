...
class Fuzzifier(object):
    ...
    def gaussianValue(self, x, mean, sigma):
    ...
    def gaussCenter(self, x, mean, sigma):
    ...
    def numbersToRowSets(self, idx, values, sigma, mean):
        ...
        if self.variables["gausses"] == 3:
            cr_middle, sigma = self.calculateSigma(mean * (1/2), mean * (3/2))
            self.features[idx][self.variables["verylow"]] = self.gaussLeft(self.x_range, mean, sigma)
            self.features[idx][self.variables["middle"]] = self.gaussianFunction(self.x_range, mean, sigma, -1, -1, 0, 1.01)
            self.features[idx][self.variables["veryhigh"]] = self.gaussRight(self.x_range, mean, sigma)

        elif self.variables["gausses"] == 5:
            cr_low, sigma = self.calculateSigma(mean * (1/2), mean)
            self.features[idx][self.variables["verylow"]] = self.gaussLeft(self.x_range, mean * (1/2), sigma)
            self.features[idx][self.variables["low"]] = self.gaussianFunction(self.x_range, mean * (1/2), sigma, -1, cr_low, 0, mean)
 
            cr_middle, sigma = self.calculateSigma(mean, mean * (3/2))
            self.features[idx][self.variables["middle"]] = self.gaussianFunction(self.x_range, mean, sigma, cr_low, cr_middle, mean * (1/2), mean * (3/2))

            self.features[idx][self.variables["high"]] = self.gaussianFunction(self.x_range, mean * (3/2), sigma, cr_middle, -1, mean, 1.01)  
            self.features[idx][self.variables["veryhigh"]] = self.gaussRight(self.x_range, mean * (3/2), sigma)

        elif self.variables["gausses"] == 7:
            cr_low, sigma = self.calculateSigma(mean * (1/3), mean * (2/3))
            self.features[idx][self.variables["verylow"]] = self.gaussLeft(self.x_range, mean * (1/3), sigma)
            self.features[idx][self.variables["low"]] = self.gaussianFunction(self.x_range, mean * (1/3), sigma, -1, cr_low, 0, mean * (2/3))
            
            cr_middlelow, sigma = self.calculateSigma(mean * (2/3), mean)
            self.features[idx][self.variables["middlelow"]] = self.gaussianFunction(self.x_range, mean * (2/3), sigma, cr_low, cr_middlelow, mean * (1/3), mean)
            
            cr_middle, sigma = self.calculateSigma(mean, mean * (4/3))
            self.features[idx][self.variables["middle"]] = self.gaussianFunction(self.x_range, mean, sigma, cr_middlelow, cr_middle, mean * (2/3), mean * (4/3))
            
            cr_middlehigh, sigma = self.calculateSigma(mean * (4/3), mean * (5/3))
            self.features[idx][self.variables["middlehigh"]] = self.gaussianFunction(self.x_range, mean * (4/3), sigma, cr_middle, cr_middlehigh, mean, mean * (5/3))
            
            self.features[idx][self.variables["high"]] = self.gaussianFunction(self.x_range, mean * (5/3), sigma, cr_middlehigh, -1, mean * (4/3), 1.01)  
            self.features[idx][self.variables["veryhigh"]] = self.gaussRight(self.x_range, mean * (5/3), sigma)
            
        elif self.variables["gausses"] == 9:

            cr_low, sigma = self.calculateSigma(mean * (1/4), mean * (2/4))
            self.features[idx][self.variables["verylow"]] = self.gaussLeft(self.x_range, mean * (1/4), sigma)
            self.features[idx][self.variables["low"]] = self.gaussianFunction(self.x_range, mean * (1/4), sigma, -1, cr_low, 0, mean * (2/4))

            cr_middlelowminus, sigma = self.calculateSigma(mean * (2/4), mean * (3/4))
            self.features[idx][self.variables["middlelowminus"]] = self.gaussianFunction(self.x_range, mean * (2/4), sigma, cr_low, cr_middlelowminus, mean * (1/4), mean * (3/4))
          
            cr_middlelow, sigma = self.calculateSigma(mean * (3/4), mean)
            self.features[idx][self.variables["middlelow"]] = self.gaussianFunction(self.x_range, mean * (3/4), sigma, cr_middlelowminus, cr_middlelow, mean * (2/4), mean)

            cr_middle, sigma = self.calculateSigma(mean, mean * (5/4))
            self.features[idx][self.variables["middle"]] = self.gaussianFunction(self.x_range, mean, sigma, cr_middlelow, cr_middle, mean * (3/4), mean * (5/4))

            cr_middlehigh, sigma = self.calculateSigma(mean * (5/4), mean * (6/4))
            self.features[idx][self.variables["middlehigh"]] = self.gaussianFunction(self.x_range, mean * (5/4), sigma, cr_middle, cr_middlehigh, mean, mean * (6/4))
            
            cr_middlehighplus, sigma = self.calculateSigma(mean * (6/4), mean * (7/4))
            self.features[idx][self.variables["middlehighplus"]] = self.gaussianFunction(self.x_range, mean * (6/4), sigma, cr_middlehigh, cr_middlehighplus, mean * (5/4), mean * (7/4))
            
            self.features[idx][self.variables["high"]] = self.gaussianFunction(self.x_range, mean * (7/4), sigma, cr_middlehighplus, -1, mean * (6/4), 1.01)  
            self.features[idx][self.variables["veryhigh"]] = self.gaussRight(self.x_range, mean * (7/4), sigma)

                        
        elif self.variables["gausses"] == 11:
            cr_low, sigma = self.calculateSigma(mean * (1/5), mean * (2/5))
            self.features[idx][self.variables["verylow"]] = self.gaussLeft(self.x_range, mean * (1/5), sigma)
            self.features[idx][self.variables["low"]] = self.gaussianFunction(self.x_range, mean * (1/5), sigma, -1, cr_low, 0, mean * (2/5))

            cr_middlelowminus, sigma = self.calculateSigma(mean * (2/5), mean * (3/5))
            self.features[idx][self.variables["middlelowminus"]] = self.gaussianFunction(self.x_range, mean * (2/5), sigma, cr_low, cr_middlelowminus, mean * (1/5), mean * (3/5))
          
            cr_middlelow, sigma = self.calculateSigma(mean * (3/5), mean * (4/5))
            self.features[idx][self.variables["middlelow"]] = self.gaussianFunction(self.x_range, mean * (3/5), sigma, cr_middlelowminus, cr_middlelow, mean * (2/5), mean * (4/5))
   
            cr_middlelowplus, sigma = self.calculateSigma(mean * (4/5), mean * (5/5))
            self.features[idx][self.variables["middlelowplus"]] = self.gaussianFunction(self.x_range, mean * (4/5), sigma, cr_middlelow, cr_middlelowplus, mean * (3/5), mean * (5/5))

            cr_middle, sigma = self.calculateSigma(mean, mean * (6/5))
            self.features[idx][self.variables["middle"]] = self.gaussianFunction(self.x_range, mean, sigma, cr_middlelowplus, cr_middle, mean * (4/5), mean * (6/5))

            cr_middlehighminus, sigma = self.calculateSigma(mean * (6/5), mean * (7/5))
            self.features[idx][self.variables["middlehighminus"]] = self.gaussianFunction(self.x_range, mean * (6/5), sigma, cr_middle, cr_middlehighminus, mean * (5/5), mean * (7/5))

            cr_middlehigh, sigma = self.calculateSigma(mean * (7/5), mean * (8/5))
            self.features[idx][self.variables["middlehigh"]] = self.gaussianFunction(self.x_range, mean * (7/5), sigma, cr_middlehighminus, cr_middlehigh, mean * (6/5), mean * (8/5))
            
            cr_middlehighplus, sigma = self.calculateSigma(mean * (8/5), mean * (9/5))
            self.features[idx][self.variables["middlehighplus"]] = self.gaussianFunction(self.x_range, mean * (8/5), sigma, cr_middlehigh, cr_middlehighplus, mean * (7/5), mean * (9/5))
            
            self.features[idx][self.variables["high"]] = self.gaussianFunction(self.x_range, mean * (9/5), sigma, cr_middlehighplus, -1, mean * (8/5), 1.01)  
            self.features[idx][self.variables["veryhigh"]] = self.gaussRight(self.x_range, mean * (9/5), sigma)
        ...
        for x in values:
            ...
            if self.variables["gausses"] == 3:
                ...
                middle_value = self.gaussianValue(x, mean, sigma)
            ...
            if self.variables["gausses"] == 5:
                ...
                low_value = self.gaussianValue(x, mean * (1/2), sigma)
                middle_value = self.gaussianValue(x, mean, sigma)
                high_value = self.gaussianValue(x, mean * (3/2), sigma)
                veryhigh_value = self.rightGaussianValue(x, mean * (3/2), sigma)

            elif self.variables["gausses"] == 7:
                verylow_value = self.leftGaussianValue(x, mean * (1/3), sigma)
                low_value = self.gaussianValue(x, mean * (1/3), sigma)
                middlelow_value = self.gaussianValue(x, mean * (2/3), sigma)
                middle_value = self.gaussianValue(x, mean, sigma)
                middlehigh_value = self.gaussianValue(x, mean * (4/3), sigma)
                high_value = self.gaussianValue(x, mean * (5/3), sigma)
                veryhigh_value = self.rightGaussianValue(x, mean * (5/3), sigma)

            elif self.variables["gausses"] == 9:
                verylow_value = self.leftGaussianValue(x, mean * (1/4), sigma)
                low_value = self.gaussianValue(x, mean * (1/4), sigma)
                middlelowminus_value = self.gaussianValue(x, mean * (2/4), sigma)
                middlelow_value = self.gaussianValue(x, mean * (3/4), sigma)
                middle_value = self.gaussianValue(x, mean, sigma)
                middlehigh_value = self.gaussianValue(x, mean * (5/4), sigma)
                middlehighplus_value = self.gaussianValue(x, mean * (6/4), sigma)
                high_value = self.gaussianValue(x, mean * (7/4), sigma)
                veryhigh_value = self.rightGaussianValue(x, mean * (7/4), sigma)
                            
            elif self.variables["gausses"] == 11:
                verylow_value = self.leftGaussianValue(x, mean * (1/5), sigma)
                low_value = self.gaussianValue(x, mean * (1/5), sigma)
                middlelowminus_value = self.gaussianValue(x, mean * (2/5), sigma)
                middlelow_value = self.gaussianValue(x, mean * (3/5), sigma)
                middlelowplus_value = self.gaussianValue(x, mean * (4/5), sigma)
                middle_value = self.gaussianValue(x, mean, sigma)
                middlehighminus_value = self.gaussianValue(x, mean * (6/5), sigma)
                middlehigh_value = self.gaussianValue(x, mean * (7/5), sigma)
                middlehighplus_value = self.gaussianValue(x, mean * (8/5), sigma)
                high_value = self.gaussianValue(x, mean * (9/5), sigma)
