...
class Fuzzifier(object):
    ...
    def numbersToThreeRowSets(self, idx, values, sigma, mean):
    ...
    def fuzzify(self, features_table, sigma_mean_params):
        if isinstance(sigma_mean_params, (int, np.integer)):
            for idx, x in enumerate(self.features):
                features_table[x.label] = self.numbersToThreeRowSets(idx, features_table[x.label], sigma_mean_params, sigma_mean_params)
        else:
            for idx, x in enumerate(self.features):
                features_table[x.label] = self.numbersToThreeRowSets(idx, features_table[x.label], sigma_mean_params[idx*2], sigma_mean_params[idx*2 + 1])
